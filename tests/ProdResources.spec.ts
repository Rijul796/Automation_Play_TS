...
async function checkDocumentLinks(page: Page, optionName = 'Unknown option'): Promise<BrokenLink[]> {
  ...
  // Collect links for common document types: pdf, doc, docx, ppt, pptx, xlsx
...
const BASE_URL = 'https://www.riversidedatamanager.com';
const USER_EMAIL = process.env.TEST_USER_EMAIL || 'rpcdm2019+pasys@gmail.com';
  ...
  const allBroken: BrokenLink[] = [];
      ...
      for (const action of dropdownActions) {
        try {
          ...
          // Check the document links and collect any broken links
          console.log(`Checking document links for option: ${optionText}`);
      ...
      if (allBroken.length > 0) {
        ...
        const red = '\u001b[31m';
        const reset = '\u001b[0m';
        console.log(`${red}\n=== BROKEN DOCUMENT LINKS SUMMARY (${allBroken.length}) ===${reset}`);
