...
export const BASE_URL = 'https://www.riversidedatamanager.com';
export const USER_EMAIL = process.env.TEST_USER_EMAIL || 'rpcdm2019+pasys@gmail.com';
...
test('user can login and check all links on directions page', async ({ page }) => {
  ...
  await test.step('Open Directions/Resources', async () => {
    ...
    if (href) {
      try {
        ...
        await newPage.goto(absolute, { waitUntil: 'networkidle' });
        target = newPage;
      } catch (err) {
